...
public class DemoNoSQLTableImage extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableImage.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKeyAndSortKey extends DemoNoSQLOperationBase {
        ...
        DemoGetWithPartitionKeyAndSortKey(final Context context) {
            super(context.getString(R.string.nosql_operation_get_by_partition_and_sort_text),
                    String.format(context.getString(R.string.nosql_operation_example_get_by_partition_and_sort_text),
                            ...
                            "image index", "1111500000"));
        }

        @Override
        public boolean executeOperation() {
            // Retrieve an item by passing the partition key using the object mapper.
            result = mapper.load(ImageDO.class, IdentityManager.getDefaultIdentityManager().getCachedUserID(), 1111500000.0);
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLImageResult(result));
    ...
    public class DemoQueryWithPartitionKeyAndSortKeyCondition extends DemoNoSQLOperationBase {
        ...
        private PaginatedQueryList<ImageDO> results;
        private Iterator<ImageDO> resultsIterator;
        ...
        @Override
        public boolean executeOperation() {
            final ImageDO itemToFind = new ImageDO();
            ...
            final DynamoDBQueryExpression<ImageDO> queryExpression = new DynamoDBQueryExpression<ImageDO>()
                    .withHashKeyValues(itemToFind)
                    .withRangeKeyCondition("image index", rangeKeyCondition)
                    .withConsistentRead(false)
                    .withLimit(RESULTS_PER_RESULT_GROUP);

            results = mapper.query(ImageDO.class, queryExpression);
            if (results != null) {
    ...
    public class DemoQueryWithPartitionKeyAndFilter extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() {
            ...
            filterExpressionAttributeNames.put("#Report ID", "Report ID");

            final Map<String, AttributeValue> filterExpressionAttributeValues = new HashMap<>();
            filterExpressionAttributeValues.put(":MinReport ID",
                    new AttributeValue().withN("1111500000"));

            final DynamoDBQueryExpression<ImageDO> queryExpression = new DynamoDBQueryExpression<ImageDO>()
                    .withHashKeyValues(itemToFind)
                    .withFilterExpression("#Report ID > :MinReport ID")
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<ImageDO> results;
        private Iterator<ImageDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            results = mapper.scan(ImageDO.class, scanExpression);
            if (results != null) {
                resultsIterator = results.iterator();
                if (resultsIterator.hasNext()) {
                    return true;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<ImageDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final ImageDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLImageResult(item));
            itemsRetrieved++;
    ...
    public void insertSampleData() throws AmazonClientException {
        ...
        final ImageDO firstItem = new ImageDO();

        firstItem.setUserId(IdentityManager.getDefaultIdentityManager().getCachedUserID());
        firstItem.setImageIndex(1111500000.0);
        firstItem.setReportID(DemoSampleDataGenerator.getRandomSampleNumber());
        firstItem.setComment(
                DemoSampleDataGenerator.getRandomSampleString("comment"));
        firstItem.setImageFile(DemoSampleDataGenerator.getRandomSampleBinary());
        ...
        final ImageDO[] items = new ImageDO[SAMPLE_DATA_ENTRIES_PER_INSERT - 1];
        for (int count = 0; count < SAMPLE_DATA_ENTRIES_PER_INSERT - 1; count++) {
            final ImageDO item = new ImageDO();
            item.setUserId(IdentityManager.getDefaultIdentityManager().getCachedUserID());
            item.setImageIndex(DemoSampleDataGenerator.getRandomSampleNumber());
            item.setReportID(DemoSampleDataGenerator.getRandomSampleNumber());
            item.setComment(DemoSampleDataGenerator.getRandomSampleString("comment"));
            item.setImageFile(DemoSampleDataGenerator.getRandomSampleBinary());
    ...
    public void removeSampleData() throws AmazonClientException {
        ...
        final PaginatedQueryList<ImageDO> results = mapper.query(ImageDO.class, queryExpression);

        Iterator<ImageDO> resultsIterator = results.iterator();
        ...
        final List<ImageDO> batchOfItems = new LinkedList<ImageDO>();
